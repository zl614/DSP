...
def find_asset(rel):
    ...
    为了不让"旁边那个旧文件静默盖掉新版"变成玄学问题，启动时会打印每个页面的实际来源。
    """
    side = os.path.join(_app_dir(), rel)          # exe / 脚本 同目录
    if os.path.exists(side):
        return side
    return resource_path(rel)                      # 打包进 exe 的（_MEIPASS）
...
# 页面文件统一用 find_asset() 解析（exe 同目录优先 → 其次 exe 内置）
PAGES = ["dashboard_realtime.html", "exception_reason_analysis.html", "fleet_splitter.html"]
...
def index():
    try:
        with open(find_asset("dashboard_realtime.html"), encoding="utf-8") as f:
...
def exceptions_page():
    try:
        with open(find_asset("exception_reason_analysis.html"), encoding="utf-8") as f:
            return Response(f.read(), mimetype="text/html; charset=utf-8")
    except FileNotFoundError:
...
if __name__ == "__main__":
    ...
    print("  页面来源：")
    for _p in PAGES:
        print(f"    {_p:32s} ← {asset_origin(_p)}")
    if ok_graph:
